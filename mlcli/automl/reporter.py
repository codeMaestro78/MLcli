...
class AutoMLReporter:
    ...
    def print_summary(self, show_params: bool = True) -> None:
        ...
        header_text = Text()
        header_text.append("🤖 AutoML Classification Report\n\n", style="bold blue")
        header_text.append(f"Metric: ", style="dim")
        header_text.append(f"{config.get('metric', 'N/A')}\n", style="cyan")
        header_text.append(f"Time Budget: ", style="dim")
        ...
        header_text.append(f"Models Evaluated: ", style="dim")
        ...
        if best.get("name"):
            ...
            best_panel_text.append(f"Model: ", style="dim")
    ...
    def print_leaderboard(self, top_n: Optional[int] = None) -> None:
        ...
        table = Table(title="🏆 AutoML Leaderboard", show_header=True)
    ...
    def _html_preprocessing_card(self, preprocessing: Dict[str, Any]) -> str:
        ...
        for warn in preprocessing.get("warnings", []):
            warnings_html += f'<p class="warning">⚠️ {warn}</p>'

        missing_info = ""
        if preprocessing.get("has_missing_values"):
            missing_info = f"""
            <p class="warning">
                ⚠️ Missing values detected. Recommended strategy:
