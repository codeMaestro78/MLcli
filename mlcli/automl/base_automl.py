...
class BaseAutoML(ABC):
    ...
    def __init__(self,task:str="classification",metric:str="accuracy",time_budget_minutes:Optional[Union[int,float]]=None,
                 random_state:Optional[int]=42,tracker:Optional[Any]=None,verbose:bool=True) ->None:
        """
    ...
    def _add_leaderboard_entry(self,model_name:str,framework:str,score:float,params:Dict[str,Any],duration_seconds:float,extra:Optional[Dict[str,Any]]=None)->None:
    ...
    def _validate_inputs(self,X:np.ndarray,y:np.ndarray)->None:
        ...
        if X.shape[0]!=y.shape[0]:
            raise ValueError(
                f"X and y must have same n_samples: {X.shape[0]} vs {y.shape[0]}"
            )
