...
class ModelSelector:
    ...
    def __init__(self,task:str="classification",
        ...
        logger.debug(f"ModelSelector initialized for {task} with {len(self._base_models)} base models")

    def select_models(self,n_samples:Optional[int]=None,
                      n_features:Optional[int]=None,
                      time_budget_minutes:Optional[int]=None,
                      max_models:Optional[int]=None)->List[ModelCandidate]:
        """
        ...
        for model_name in self._base_models:
            ...
            logger.info(f"Selected {len(candidates)} candidates models: {[c.name for c in candidates]}")
        return candidates
